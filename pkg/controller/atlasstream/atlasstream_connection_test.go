...
func TestStreamConnectionToAtlas(t *testing.T) {
	...
	t.Run("should return error to map a kafka configuration when fail to get authentication data", func(t *testing.T) {
		testScheme := runtime.NewScheme()
		assert.NoError(t, akov2.AddToScheme(testScheme))
		assert.NoError(t, corev1.AddToScheme(testScheme))
	...
	t.Run("should return error to map a kafka configuration when fail to get certificate data", func(t *testing.T) {
		testScheme := runtime.NewScheme()
		assert.NoError(t, akov2.AddToScheme(testScheme))
		assert.NoError(t, corev1.AddToScheme(testScheme))
