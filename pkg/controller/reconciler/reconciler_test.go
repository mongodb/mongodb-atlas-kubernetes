...
func TestSolveCredentials(t *testing.T) {
	ctx := context.Background()
	for _, tc := range []struct {
		title         string
		objects       []client.Object
		input         akov2.ProjectReferrerObject
		...
		t.Run(tc.title, func(t *testing.T) {
			...
			credential, err := r.SolveCredentials(ctx, tc.input)
...
func mergeSchemaObject(objs []client.Object, pro akov2.ProjectReferrerObject) []client.Object {
