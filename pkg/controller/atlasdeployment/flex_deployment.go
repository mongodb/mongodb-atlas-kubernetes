...
func (r *AtlasDeploymentReconciler) handleFlexInstance(ctx *workflow.Context, deploymentInAKO, deploymentInAtlas *deployment.Flex) (ctrl.Result, error) {
	if deploymentInAtlas == nil {
		ctx.Log.Infof("Flex Instance %s doesn't exist in Atlas - creating", deploymentInAKO.GetName())
		newFlexDeployment, err := r.deploymentService.CreateDeployment(ctx.Context, deploymentInAKO)
		if err != nil {
