...
func (r *AtlasDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	credentials, err := r.SolveCredentials(workflowCtx.Context, atlasDeployment)
	if err != nil {
	...
	atlasProject, err := r.SolveProject(workflowCtx.Context, sdkClient, atlasDeployment, orgID)
	if err != nil {
		return r.terminate(workflowCtx, workflow.AtlasAPIAccessNotConfigured, err)
	}
