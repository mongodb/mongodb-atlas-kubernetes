...
func (r *AtlasProjectReconciler) ensureProjectExists(ctx *workflow.Context, project *mdbv1.AtlasProject) (string, bool, workflow.Result) {
	isNew := false
	...
	if err != nil {
		...
		if errors.As(err, &apiError) && (apiError.ErrorCode == atlas.NotInGroup || apiError.ErrorCode == atlas.ResourceNotFound) {
			...
			if p, _, err = ctx.Client.Projects.Create(context.Background(), p, &mongodbatlas.CreateProjectOptions{}); err != nil {
				return "", isNew, workflow.Terminate(workflow.ProjectNotCreatedInAtlas, err.Error())
			}
	...
	if p == nil || p.ID == "" {
		...
		return "", isNew, workflow.Terminate(workflow.Internal, "")
	}

	return p.ID, isNew, workflow.OK()
}
