...
import (
	...
	akov1 "github.com/mongodb/mongodb-atlas-kubernetes/v2/pkg/api/v1"
...
func (r *AtlasProjectReconciler) ensureDeletionFinalizer(workflowCtx *workflow.Context, atlasClient *mongodbatlas.Client, project *akov1.AtlasProject) (result workflow.Result) {
	log := workflowCtx.Log
...
func (r *AtlasProjectReconciler) ensureProjectResources(workflowCtx *workflow.Context, project *akov1.AtlasProject) (results []workflow.Result) {
...
func (r *AtlasProjectReconciler) deleteAtlasProject(ctx context.Context, atlasClient *mongodbatlas.Client, project *akov1.AtlasProject) (err error) {
...
func (r *AtlasProjectReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		Named("AtlasProject").
		For(&akov1.AtlasProject{}, builder.WithPredicates(r.GlobalPredicates...)).
		For(&v1alpha1.AtlasProject{}, builder.WithPredicates(r.GlobalPredicates...)).
		Watches(&corev1.Secret{}, watch.NewSecretHandler(r.WatchedResources)).
		Watches(&akov1.AtlasTeam{}, watch.NewAtlasTeamHandler(r.WatchedResources)).
		Complete(r)
}
...
func managedByAtlas(workflowCtx *workflow.Context) customresource.AtlasChecker {
	return func(resource akov1.AtlasCustomResource) (bool, error) {
		project, ok := resource.(*akov1.AtlasProject)
		if !ok {
