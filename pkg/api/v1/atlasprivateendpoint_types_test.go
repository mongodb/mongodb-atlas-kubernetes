package v1

import (
	"testing"

	"github.com/mongodb/mongodb-atlas-kubernetes/v2/pkg/api"
	"github.com/mongodb/mongodb-atlas-kubernetes/v2/pkg/api/v1/common"
)

func TestPrivateEndpointProjectReference(t *testing.T) {
	tests := celTestCase{
		"no project reference is set": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{},
			},
			expectedErrors: []string{"spec: Invalid value: \"object\": must define only one project reference through externalProjectRef or projectRef"},
		},
		"both project references are set": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					ExternalProject: &ExternalProjectReference{
						ID: "my-project-id",
					},
				},
			},
			expectedErrors: []string{
				"spec: Invalid value: \"object\": must define only one project reference through externalProjectRef or projectRef",
				"spec: Invalid value: \"object\": must define a local connection secret when referencing an external project",
			},
		},
		"external project references is set": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					ExternalProject: &ExternalProjectReference{
						ID: "my-project-id",
					},
				},
			},
			expectedErrors: []string{
				"spec: Invalid value: \"object\": must define a local connection secret when referencing an external project",
			},
		},
		"kubernetes project references is set": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
				},
			},
		},
	}

	assertCELValidation(t, "../../../config/crd/bases/atlas.mongodb.com_atlasprivateendpoints.yaml", tests)
}

func TestPrivateEndpointExternalProjectReferenceConnectionSecret(t *testing.T) {
	tests := celTestCase{
		"external project references is set without connection secret": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					ExternalProject: &ExternalProjectReference{
						ID: "my-project-id",
					},
				},
			},
			expectedErrors: []string{
				"spec: Invalid value: \"object\": must define a local connection secret when referencing an external project",
			},
		},
		"external project references is set with connection secret": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					ExternalProject: &ExternalProjectReference{
						ID: "my-project-id",
					},
					LocalCredentialHolder: api.LocalCredentialHolder{
						ConnectionSecret: &api.LocalObjectReference{
							Name: "my-dbuser-connection-secret",
						},
					},
				},
			},
		},
		"kubernetes project references is set without connection secret": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
				},
			},
		},
		"kubernetes project references is set with connection secret": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					LocalCredentialHolder: api.LocalCredentialHolder{
						ConnectionSecret: &api.LocalObjectReference{
							Name: "my-dbuser-connection-secret",
						},
					},
				},
			},
		},
	}

	assertCELValidation(t, "../../../config/crd/bases/atlas.mongodb.com_atlasprivateendpoints.yaml", tests)
}

func TestPrivateEndpointProviderConfiguration(t *testing.T) {
	tests := celTestCase{
		"AWS private endpoint create new configuration": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AWS",
				},
			},
		},
		"AWS private endpoint update with missing configuration": {
			oldObject: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AWS",
				},
			},
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AWS",
				},
			},
			expectedErrors: []string{"spec: Invalid value: \"object\": must define a configuration for AWS provider"},
		},
		"AWS private endpoint update with configuration": {
			oldObject: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AWS",
				},
			},
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AWS",
					AWSConfiguration: &AWSPrivateEndpointConfiguration{
						ID: "aws-pe-id",
					},
				},
			},
		},
		"Azure private endpoint create new configuration": {
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AZURE",
				},
			},
		},
		"Azure private endpoint update with missing configuration": {
			oldObject: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AZURE",
				},
			},
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AZURE",
				},
			},
			expectedErrors: []string{"spec: Invalid value: \"object\": must define a configuration for Azure provider"},
		},
		"Azure private endpoint update with configuration": {
			oldObject: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AZURE",
				},
			},
			object: &AtlasPrivateEndpoint{
				Spec: AtlasPrivateEndpointSpec{
					Project: &common.ResourceRefNamespaced{
						Name: "my-project",
					},
					Provider: "AZURE",
					AzureConfiguration: &AzurePrivateEndpointConfiguration{
						ID: "aws-pe-id",
						IP: "10.0.0.1",
					},
				},
			},
		},
	}

	assertCELValidation(t, "../../../config/crd/bases/atlas.mongodb.com_atlasprivateendpoints.yaml", tests)
}
