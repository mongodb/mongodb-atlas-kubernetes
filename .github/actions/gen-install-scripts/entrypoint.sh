...
cd config/manager && go tool kustomize edit set image controller="${INPUT_IMAGE_URL}"
cd -
./scripts/split_roles_yaml.sh

which go tool kustomize
go tool kustomize version

# all-in-one
go tool kustomize build --load-restrictor LoadRestrictionsNone "config/release/${INPUT_ENV}/allinone" >"${target_dir}/all-in-one.yaml"
echo "Created all-in-one config"

# clusterwide
go tool kustomize build --load-restrictor LoadRestrictionsNone "config/release/${INPUT_ENV}/clusterwide" >"${clusterwide_dir}/clusterwide-config.yaml"
go tool kustomize build "config/crd" >"${clusterwide_dir}/crds.yaml"
echo "Created clusterwide config"

# base-openshift-namespace-scoped
go tool kustomize build --load-restrictor LoadRestrictionsNone "config/release/${INPUT_ENV}/openshift" >"${openshift}/openshift.yaml"
go tool kustomize build "config/crd" >"${openshift}/crds.yaml"
echo "Created openshift namespaced config"

# namespaced
go tool kustomize build --load-restrictor LoadRestrictionsNone "config/release/${INPUT_ENV}/namespaced" >"${namespaced_dir}/namespaced-config.yaml"
go tool kustomize build "config/crd" >"${namespaced_dir}/crds.yaml"
