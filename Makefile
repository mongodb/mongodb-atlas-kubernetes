...
.PHONY: fail-on-dirty
fail-on-dirty: ## Fail if the git repository is dirty
	@if ! git diff --quiet; then \
		git status; \
		echo "FAILURE: You have unstaged changes."; \
		exit 1; \
	elif [ -n "$$(git ls-files --others --exclude-standard)" ]; then \
		git status; \
		echo "FAILURE: You have untracked files."; \
		exit 1; \
	fi
	@echo "✅ Git repository is clean. No unstaged, untracked, or uncommitted changes."
