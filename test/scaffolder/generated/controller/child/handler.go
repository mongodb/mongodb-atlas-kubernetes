...
func (h *Handler) getHandlerForResource(ctx context.Context, child *akov2generated.Child) (ctrlstate.StateHandler[akov2generated.Child], error) {
	...
	var selectedHandler ctrlstate.StateHandler[akov2generated.Child]

	if child.Spec.V20250312 != nil {
		translator, ok := h.translators["v20250312"]
		if ok != true {
			return nil, errors.New("unsupported version v20250312 set in CR")
		}
		versionCount++
		selectedHandler = h.handlerv20250312(h.Client, atlasClients.SdkClient20250312014, translator, h.deletionProtection)
	}
...
func (h *Handler) HandleInitial(ctx context.Context, child *akov2generated.Child) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, child)
	if err != nil {
		return result.Error(state.StateInitial, err)
	}
	return handler.HandleInitial(ctx, child)
}

// HandleImportRequested delegates to the version-specific handler
func (h *Handler) HandleImportRequested(ctx context.Context, child *akov2generated.Child) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, child)
	if err != nil {
		return result.Error(state.StateImportRequested, err)
	}
	return handler.HandleImportRequested(ctx, child)
}

// HandleImported delegates to the version-specific handler
func (h *Handler) HandleImported(ctx context.Context, child *akov2generated.Child) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, child)
	if err != nil {
		return result.Error(state.StateImported, err)
	}
	return handler.HandleImported(ctx, child)
}

// HandleCreating delegates to the version-specific handler
func (h *Handler) HandleCreating(ctx context.Context, child *akov2generated.Child) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, child)
	if err != nil {
		return result.Error(state.StateCreating, err)
	}
	return handler.HandleCreating(ctx, child)
}

// HandleCreated delegates to the version-specific handler
func (h *Handler) HandleCreated(ctx context.Context, child *akov2generated.Child) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, child)
	if err != nil {
		return result.Error(state.StateCreated, err)
	}
	return handler.HandleCreated(ctx, child)
}

// HandleUpdating delegates to the version-specific handler
func (h *Handler) HandleUpdating(ctx context.Context, child *akov2generated.Child) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, child)
	if err != nil {
		return result.Error(state.StateUpdating, err)
	}
	return handler.HandleUpdating(ctx, child)
}

// HandleUpdated delegates to the version-specific handler
func (h *Handler) HandleUpdated(ctx context.Context, child *akov2generated.Child) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, child)
	if err != nil {
		return result.Error(state.StateUpdated, err)
	}
	return handler.HandleUpdated(ctx, child)
}
...
func (h *Handler) SetupWithManager(mgr controllerruntime.Manager, rec reconcile.Reconciler, defaultOptions controller.Options) error {
	...
	return controllerruntime.NewControllerManagedBy(mgr).Named("Child").For(h.For()).Watches(&akov2generated.Parent{}, handler.EnqueueRequestsFromMapFunc(indexers.NewChildByParentMapFunc(h.Client)), builder.WithPredicates(predicate.ResourceVersionChangedPredicate{})).WithOptions(defaultOptions).Complete(rec)
}

// getSDKClientSet creates an Atlas SDK client set using credentials from the resource's connection secret
func (h *Handler) getSDKClientSet(ctx context.Context, child *akov2generated.Child) (*atlas.ClientSet, error) {
	var connectionSecretRef *client.ObjectKey
	if child.Spec.ConnectionSecretRef != nil {
		connectionSecretRef = &client.ObjectKey{
			Name:      child.Spec.ConnectionSecretRef.Name,
