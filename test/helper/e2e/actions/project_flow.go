...
func CreateProjectWithCloudProviderAccess(testData *model.TestDataProvider, atlasIAMRoleName string) {
	...
	By("Configure cloud provider access", func() {
		...
		roleArn, err := testData.AWSResourcesGenerator.CreateIAMRole(atlasIAMRoleName, func() aws.IAMPolicy {
			cloudProviderAccess := testData.Project.Status.CloudProviderAccessRoles[0]
			return aws.CloudProviderAccessPolicy(cloudProviderAccess.AtlasAWSAccountArn, cloudProviderAccess.AtlasAssumedRoleExternalID)
		})
