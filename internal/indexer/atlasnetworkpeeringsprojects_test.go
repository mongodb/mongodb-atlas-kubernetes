...
func TestAtlasNetworkPeeringByProjectIndices(t *testing.T) {
	t.Run("should return nil when instance has no project associated to it", func(t *testing.T) {
		pe := &akov2.AtlasNetworkPeering{
			Spec: akov2.AtlasNetworkPeeringSpec{},
		}

		indexer := NewAtlasNetworkPeeringByProjectIndexer(zaptest.NewLogger(t))
