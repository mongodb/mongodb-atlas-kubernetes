	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			k8sClient := fake.NewClientBuilder().
				...
				Build()
			service := customroles.NewCustomRoles(tt.args.ctx.SdkClientSet.SdkClientLatest.CustomDatabaseRolesApi)
...
func solveProjectID(t *testing.T, r *AtlasCustomRoleReconciler, args args) (*project.Project, error) {
	...
	return r.ResolveProject(args.ctx.Context, args.ctx.SdkClientSet.SdkClientLatest, args.akoCustomRole)
}
