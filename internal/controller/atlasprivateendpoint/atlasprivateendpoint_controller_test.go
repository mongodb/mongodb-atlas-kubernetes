...
func TestEnsureCustomResource(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"failed to get project from cluster": {
			...
			expectedLogs: []string{
				"resource 'pe1' version is valid",
				"resource *v1.AtlasPrivateEndpoint(default/pe1) failed on condition Ready: error resolving project reference: AtlasProject not found: atlasprojects.atlas.mongodb.com \"my-project\" not found",
				"Status update",
			},
		},
