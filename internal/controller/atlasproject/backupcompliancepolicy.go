...
func (b *backupComplianceController) upsert(atlasBCP *admin.DataProtectionSettings20231001) workflow.DeprecatedResult {
	...
	if !equal {
		atlasBCP, _, err = b.ctx.SdkClientSet.SdkClient20250312.CloudBackupsApi.UpdateCompliancePolicy(b.ctx.Context, b.project.ID(), akoBCP.ToAtlas(b.project.ID())).OverwriteBackupPolicies(akoBCP.Spec.OverwriteBackupPolicies).Execute()
		if err != nil {
...
func (b *backupComplianceController) getAtlasBackupCompliancePolicy() (*admin.DataProtectionSettings20231001, bool, error) {
	bcp, _, err := b.ctx.SdkClientSet.SdkClient20250312.CloudBackupsApi.GetCompliancePolicy(b.ctx.Context, b.project.ID()).Execute()
	if err != nil {
