...
func patchSettings(ctx *workflow.Context, projectID string, spec *akov2.ProjectSettings) error {
	...
	_, _, err := ctx.SdkClientSet.SdkClient20250312.ProjectsApi.UpdateGroupSettings(ctx.Context, projectID, specAsAtlas).Execute()
	return err
}

func fetchSettings(ctx *workflow.Context, projectID string) (*akov2.ProjectSettings, error) {
	data, _, err := ctx.SdkClientSet.SdkClient20250312.ProjectsApi.GetGroupSettings(ctx.Context, projectID).Execute()
	if err != nil {
		return nil, err
	}
