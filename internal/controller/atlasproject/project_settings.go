...
func patchSettings(ctx *workflow.Context, projectID string, spec *akov2.ProjectSettings) error {
	...
	_, _, err := ctx.SdkClientSet.SdkClientLatest.ProjectsApi.UpdateGroupSettings(ctx.Context, projectID, specAsAtlas).Execute()
	return err
}

func fetchSettings(ctx *workflow.Context, projectID string) (*akov2.ProjectSettings, error) {
	data, _, err := ctx.SdkClientSet.SdkClientLatest.ProjectsApi.GetGroupSettings(ctx.Context, projectID).Execute()
	if err != nil {
		return nil, err
	}
