...
func (r *AtlasDataFederationReconciler) Reconcile(context context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	endpointService := datafederation.NewDatafederationPrivateEndpoint(clientSet.SdkClient20250312012.DataFederationApi)
	dataFederationService := datafederation.NewAtlasDataFederation(clientSet.SdkClient20250312012.DataFederationApi)
	...
	if !dataFederation.GetDeletionTimestamp().IsZero() {
		return r.handleDelete(ctx, log, dataFederation, project, dataFederationService).ReconcileResult()
	}
