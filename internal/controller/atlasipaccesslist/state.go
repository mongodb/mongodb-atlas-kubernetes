...
func (r *AtlasIPAccessListReconciler) handleCustomResource(ctx context.Context, ipAccessList *akov2.AtlasIPAccessList) ctrl.Result {
	...
	atlasProject, err := r.ResolveProject(ctx, sdkClientSet.SdkClient20250312002, ipAccessList)
	if err != nil {
		return r.terminate(workflowCtx, ipAccessList, api.ReadyType, workflow.AtlasAPIAccessNotConfigured, err)
	}
	ipAccessListService := ipaccesslist.NewIPAccessList(sdkClientSet.SdkClient20250312002.ProjectIPAccessListApi)
