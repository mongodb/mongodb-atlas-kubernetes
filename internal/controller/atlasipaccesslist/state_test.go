	...
	tests := map[string]struct {
		...
		"should fail to resolve credentials": {
			...
			expectedConditions: []api.Condition{
				api.FalseCondition(api.ReadyType).
					...
					WithMessageRegexp(`error resolving project reference: AtlasProject not found: atlasprojects.atlas.mongodb.com "my-no-existing-project" not found`),
				api.TrueCondition(api.ResourceVersionStatus),
			},
		},
		...
		"should fail to resolve project": {
			...
			expectedConditions: []api.Condition{
				api.FalseCondition(api.ReadyType).
					WithReason(string(workflow.AtlasAPIAccessNotConfigured)).
					WithMessageRegexp(`failed to get project via Kubernetes reference: AtlasProject not found: atlasprojects.atlas.mongodb.com "my-no-existing-project" not found`),
				api.TrueCondition(api.ResourceVersionStatus),
			},
		},
