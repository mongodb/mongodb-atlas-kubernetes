...
func setupDeploymentTest(t *testing.T) (*ConnSecretReconciler, *akov2.AtlasDeployment, *akov2.AtlasDeployment) {
	t.Helper()

	// Returns a valid Deployment
	dep := &akov2.AtlasDeployment{
	...
	// Returns a valid Deployment with an external Ref
	dep_external := &akov2.AtlasDeployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-deployment-ext",
			Namespace: "test-ns",
		},
		Spec: akov2.AtlasDeploymentSpec{
			ProjectDualReference: akov2.ProjectDualReference{
...
func TestDeploymentEndpoint_GetName(t *testing.T) {
	...
	dep := &akov2.AtlasDeployment{
		...
		Spec: akov2.AtlasDeploymentSpec{
			DeploymentSpec: &akov2.AdvancedDeploymentSpec{Name: "my-depl-name"},
		},
	}
	e := DeploymentEndpoint{obj: dep}
	assert.Equal(t, "my-depl-name", e.GetName())
}
...
func TestDeploymentEndpoint_GetProjectID(t *testing.T) {
	r, dep, dep_external := setupDeploymentTest(t)
	...
	tests := map[string]struct {
		...
		"success: external project ID": {
			endpoint: DeploymentEndpoint{obj: dep_external, r: r},
			want:     "test-project-id",
		},
		"success: k8s project ref": {
			endpoint: DeploymentEndpoint{obj: dep, r: r},
			want:     "test-project-id",
		},
	}
...
func TestDeploymentEndpoint_GetProjectName(t *testing.T) {
	r, dep, dep_external := setupDeploymentTest(t)
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			got, err := tc.endpoint.GetProjectName(context.Background())
			if tc.wantErr {
				assert.Error(t, err)
				return
			}
...
func TestDeploymentEndpoint_BuildConnData(t *testing.T) {
	r, dep, _ := setupDeploymentTest(t)

	user := &akov2.AtlasDatabaseUser{
		ObjectMeta: metav1.ObjectMeta{Name: "test-user", Namespace: "test-ns"},
	...
	dep.Status.ConnectionStrings = &status.ConnectionStrings{
