...
func setupFederationTest(t *testing.T) (*ConnSecretReconciler, *akov2.AtlasDataFederation) {
	t.Helper()

	// Returns a valid Federation
...
func TestFederationEndpoint_SelectorByProject(t *testing.T) {
	...
	assert.True(t, s.Matches(fields.Set{indexer.AtlasDataFederationByProject: "p123"}))
	assert.False(t, s.Matches(fields.Set{indexer.AtlasDataFederationByProject: "other"}))
}
...
func TestFederationEndpoint_BuildConnData(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			assert.Equal(t, "theuser", got.DBUserName)
