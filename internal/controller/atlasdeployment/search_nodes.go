...
func (s *searchNodeController) create() workflow.DeprecatedResult {
	...
	resp, _, err := s.ctx.SdkClientSet.SdkClient20250312014.AtlasSearchApi.CreateClusterSearchDeployment(s.ctx.Context, s.projectID, s.deployment.GetDeploymentName(), &admin.ApiSearchDeploymentRequest{
...
func (s *searchNodeController) update(atlasNodes *admin.ApiSearchDeploymentResponse) workflow.DeprecatedResult {
	...
	if !reflect.DeepEqual(currentAkoNodesAsAtlas, atlasNodes.GetSpecs()) {
		updateResponse, _, err := s.ctx.SdkClientSet.SdkClient20250312014.AtlasSearchApi.UpdateClusterSearchDeployment(
...
func (s *searchNodeController) delete() workflow.DeprecatedResult {
	...
	_, err := s.ctx.SdkClientSet.SdkClient20250312014.AtlasSearchApi.DeleteClusterSearchDeployment(s.ctx.Context, s.projectID, s.deployment.GetDeploymentName()).Execute()
	if err != nil {
...
func (s *searchNodeController) getAtlasSearchDeployment() (*admin.ApiSearchDeploymentResponse, bool, error) {
	atlasNodes, _, err := s.ctx.SdkClientSet.SdkClient20250312014.AtlasSearchApi.GetClusterSearchDeployment(s.ctx.Context, s.projectID, s.deployment.GetDeploymentName()).Execute()
	if err != nil {
