...
func (r *AtlasDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	projectService := project.NewProjectAPIService(sdkClientSet.SdkClient20250312012.ProjectsApi)
	deploymentService := deployment.NewAtlasDeployments(sdkClientSet.SdkClient20250312012.ClustersApi, sdkClientSet.SdkClient20250312012.GlobalClustersApi, sdkClientSet.SdkClient20250312012.FlexClustersApi, r.AtlasProvider.IsCloudGov())
	atlasProject, err := r.ResolveProject(workflowCtx.Context, sdkClientSet.SdkClient20250312012, atlasDeployment)
	if err != nil {
		return r.terminate(workflowCtx, workflow.AtlasAPIAccessNotConfigured, err)
	}
