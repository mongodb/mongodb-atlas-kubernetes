...
func (r *AtlasFederatedAuthReconciler) ensureFederatedAuth(service *workflow.Context, fedauth *akov2.AtlasFederatedAuth) workflow.DeprecatedResult {
	...
	identityProvider, err := GetIdentityProviderForFederatedSettings(service.Context, service.SdkClientSet.SdkClient20250312, atlasFedSettings)
	if err != nil {
		return workflow.Terminate(workflow.FederatedAuthNotAvailable, err)
	}
	...
	projectList, err := prepareProjectList(service.Context, service.SdkClientSet.SdkClient20250312)
	if err != nil {
	...
	if result := r.ensureIDPSettings(service.Context, atlasFedSettings.GetId(), identityProvider, fedauth, service.SdkClientSet.SdkClient20250312); !result.IsOk() {
		return result
	}
