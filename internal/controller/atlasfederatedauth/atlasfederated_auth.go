...
func (r *AtlasFederatedAuthReconciler) ensureFederatedAuth(service *workflow.Context, fedauth *akov2.AtlasFederatedAuth) workflow.DeprecatedResult {
	...
	atlasFedSettings, _, err := service.SdkClientSet.SdkClient20250312009.FederatedAuthenticationApi.
	...
	identityProvider, err := GetIdentityProviderForFederatedSettings(service.Context, service.SdkClientSet.SdkClient20250312009, atlasFedSettings)
	if err != nil {
		return workflow.Terminate(workflow.FederatedAuthNotAvailable, err)
	}

	// Get current Org config
	orgConfig, _, err := service.SdkClientSet.SdkClient20250312009.FederatedAuthenticationApi.
	...
	projectList, err := prepareProjectList(service.Context, service.SdkClientSet.SdkClient20250312009)
	if err != nil {
	...
	if result := r.ensureIDPSettings(service.Context, atlasFedSettings.GetId(), identityProvider, fedauth, service.SdkClientSet.SdkClient20250312009); !result.IsOk() {
		return result
	}
	...
	updatedSettings, _, err := service.SdkClientSet.SdkClient20250312009.FederatedAuthenticationApi.
