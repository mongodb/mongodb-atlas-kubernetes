				...
				Results: []admin.ClusterDescription20240805{
					{
						...
						ConnectionStrings: &admin.ClusterConnectionStrings{StandardSrv: new("clusterSRV")},
					},
				},
		...
		mockFlexAPI.EXPECT().ListFlexClustersExecute(
			...
			&admin.PaginatedFlexClusters20241113{
				Results: []admin.FlexClusterDescription20241113{
					{
						Name:              new("testFlex"),
						ConnectionStrings: &admin.FlexConnectionStrings20241113{StandardSrv: new("flexSRV")},
					},
				},
			}, &http.Response{StatusCode: http.StatusOK}, nil)
		...
		"should return process args": {
			...
			apiMocker: func() (admin.ClustersApi, admin.FlexClustersApi) {
				...
				clusterAPI.EXPECT().GetProcessArgsExecute(mock.AnythingOfType("admin.GetProcessArgsApiRequest")).
					Return(
						&admin.ClusterDescriptionProcessArgs20240805{
							DefaultWriteConcern:              new("available"),
							JavascriptEnabled:                new(false),
							MinimumEnabledTlsProtocol:        new("TLS1_1"),
							NoTableScan:                      new(true),
							OplogMinRetentionHours:           new(12.0),
							OplogSizeMB:                      new(5),
							SampleRefreshIntervalBIConnector: new(10),
							SampleSizeBIConnector:            new(5),
						},
						nil,
						nil,
					)
			...
			result: &Cluster{
				ProcessArgs: &akov2.ProcessArgs{
					...
					JavascriptEnabled:                new(true),
					NoTableScan:                      new(false),
					OplogSizeMB:                      new(int64(5)),
					SampleSizeBIConnector:            new(int64(5)),
					SampleRefreshIntervalBIConnector: new(int64(10)),
	...
	tests := map[string]struct {
		...
		"should upgrade flex instance in atlas": {
			...
			apiMocker: func() (admin.ClustersApi, admin.FlexClustersApi) {
				...
				flexAPI.EXPECT().TenantUpgradeExecute(mock.AnythingOfType("admin.TenantUpgradeApiRequest")).
					Return(
						&admin.FlexClusterDescription20241113{GroupId: new("project-id")},
						&http.Response{},
						nil,
					)
				return clusterAPI, flexAPI
...
func atlasGeoShardedCluster() *admin.ClusterDescription20240805 {
	return &admin.ClusterDescription20240805{
		...
		Name:                         new("cluster0"),
		ClusterType:                  new("GEOSHARDED"),
		BackupEnabled:                new(true),
		PitEnabled:                   new(true),
		Paused:                       new(false),
		TerminationProtectionEnabled: new(true),
		EncryptionAtRestProvider:     new("AWS"),
		...
		ReplicationSpecs: &[]admin.ReplicationSpec20240805{
			{
				Id:       new("replication-id-2"),
				ZoneName: new("Zone 2"),
				RegionConfigs: &[]admin.CloudRegionConfig20240805{
					{
						ProviderName: new("AWS"),
						RegionName:   new("EU_CENTRAL_1"),
						Priority:     new(6),
						ElectableSpecs: &admin.HardwareSpec20240805{
							InstanceSize:  new("M30"),
							NodeCount:     new(2),
							EbsVolumeType: new("STANDARD"),
							DiskIOPS:      new(3000),
							DiskSizeGB:    new(40.0),
						},
		...
		MongoDBVersion: new("7.3.3"),
		ConnectionStrings: &admin.ClusterConnectionStrings{
			PrivateEndpoint: &[]admin.ClusterDescriptionConnectionStringsPrivateEndpoint{
				{
					ConnectionString:                  new("connection-str"),
					SrvConnectionString:               new("connection-srv-str"),
					SrvShardOptimizedConnectionString: new("connection-sharded-srv-str"),
					Endpoints: &[]admin.ClusterDescriptionConnectionStringsPrivateEndpointEndpoint{
						{
							ProviderName: new("AWS"),
							Region:       new("US_EAST_1"),
							EndpointId:   new("arn-endpoint-id"),
						},
					},
					Type: new("MONGOS"),
				},
			},
			Private:     new("private-str"),
			PrivateSrv:  new("private-srv-str"),
			Standard:    new("standard-str"),
			StandardSrv: new("standard-srv-str"),
		},
	}
}
