...
func fuzzIntegration(fuzzer *gofuzz.Fuzzer, index uint, integration *ThirdPartyIntegration) {
	...
	if integration.Type == "DATADOG" {
		...
		integration.Datadog.APIKeySecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
	...
	if integration.Type == "MICROSOFT_TEAMS" {
		integration.MicrosoftTeams.URLSecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
	...
	if integration.Type == "NEW_RELIC" {
		integration.NewRelic.CredentialsSecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
	...
	if integration.Type == "OPS_GENIE" {
		integration.OpsGenie.APIKeySecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
	...
	if integration.Type == "PAGER_DUTY" {
		integration.PagerDuty.ServiceKeySecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
	...
	if integration.Type == "PROMETHEUS" {
		...
		integration.Prometheus.PrometheusCredentialsSecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
	...
	if integration.Type == "SLACK" {
		integration.Slack.APITokenSecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
	...
	if integration.Type == "VICTOR_OPS" {
		integration.VictorOps.APIKeySecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
	...
	if integration.Type == "WEBHOOK" {
		integration.Webhook.URLSecret.Name = "" // not part of the atlas conversion roundtrip
	} else {
