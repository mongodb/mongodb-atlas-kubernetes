...
func (h *Handler) getHandlerForResource(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.StateHandler[akov2generated.IPAccessListEntry], error) {
	atlasClients, err := h.getSDKClientSet(ctx, ipaccesslistentry)
	if err != nil {
		return nil, err
	}
	// Check which resource spec version is set and validate that only one is specified
	var versionCount int
	var selectedHandler ctrlstate.StateHandler[akov2generated.IPAccessListEntry]
...
func (h *Handler) HandleInitial(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateInitial, err)
	}
	return handler.HandleInitial(ctx, ipaccesslistentry)
}

// HandleImportRequested delegates to the version-specific handler
func (h *Handler) HandleImportRequested(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateImportRequested, err)
	}
	return handler.HandleImportRequested(ctx, ipaccesslistentry)
}

// HandleImported delegates to the version-specific handler
func (h *Handler) HandleImported(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateImported, err)
	}
	return handler.HandleImported(ctx, ipaccesslistentry)
}

// HandleCreating delegates to the version-specific handler
func (h *Handler) HandleCreating(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateCreating, err)
	}
	return handler.HandleCreating(ctx, ipaccesslistentry)
}

// HandleCreated delegates to the version-specific handler
func (h *Handler) HandleCreated(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateCreated, err)
	}
	return handler.HandleCreated(ctx, ipaccesslistentry)
}

// HandleUpdating delegates to the version-specific handler
func (h *Handler) HandleUpdating(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateUpdating, err)
	}
	return handler.HandleUpdating(ctx, ipaccesslistentry)
}

// HandleUpdated delegates to the version-specific handler
func (h *Handler) HandleUpdated(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateUpdated, err)
	}
	return handler.HandleUpdated(ctx, ipaccesslistentry)
}

// HandleDeletionRequested delegates to the version-specific handler
func (h *Handler) HandleDeletionRequested(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateDeletionRequested, err)
	}
	return handler.HandleDeletionRequested(ctx, ipaccesslistentry)
}

// HandleDeleting delegates to the version-specific handler
func (h *Handler) HandleDeleting(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (ctrlstate.Result, error) {
	handler, err := h.getHandlerForResource(ctx, ipaccesslistentry)
	if err != nil {
		return result.Error(state.StateDeleting, err)
	}
	return handler.HandleDeleting(ctx, ipaccesslistentry)
}

// For returns the resource and predicates for the controller
func (h *Handler) For() (client.Object, builder.Predicates) {
	obj := &akov2generated.IPAccessListEntry{}
...
func (h *Handler) SetupWithManager(mgr controllerruntime.Manager, rec reconcile.Reconciler, defaultOptions controller.Options) error {
	...
	return controllerruntime.NewControllerManagedBy(mgr).Named("IPAccessListEntry").For(h.For()).Watches(&akov2generated.Group{}, handler.EnqueueRequestsFromMapFunc(indexers.NewIPAccessListEntryByGroupMapFunc(h.Client)), builder.WithPredicates(predicate.ResourceVersionChangedPredicate{})).WithOptions(defaultOptions).Complete(rec)
}

// getSDKClientSet creates an Atlas SDK client set using credentials from the resource's connection secret
func (h *Handler) getSDKClientSet(ctx context.Context, ipaccesslistentry *akov2generated.IPAccessListEntry) (*atlas.ClientSet, error) {
	var connectionSecretRef *client.ObjectKey
	if ipaccesslistentry.Spec.ConnectionSecretRef != nil {
		connectionSecretRef = &client.ObjectKey{
			Name:      ipaccesslistentry.Spec.ConnectionSecretRef.Name,
			Namespace: ipaccesslistentry.Namespace,
		}
	}
